...
+
When _--print_ is specified, outputs the generated lockfile to standard
output instead of writing it to _shard.lock_.
+
...
*run* [_<target>_] [-- _<options>_...]::
Builds and runs the specified _<target>_. If no target is specified and
_shard.yml_ defines only one target, that target is run.
This command ensures all dependencies are installed, so it is not necessary
to run *shards install* before.
+
Arguments following _--_ are passed directly to the target process.

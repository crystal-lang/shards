...
+
WARNING: A _postinstall_ script should never run nested _shards_ commands
(such as `shards install`, `shards build`, or `shards check`). Nested invocations
can overwrite dependencies in the parent project's _lib_ folder and cause lockfile
mismatches.
+
*Example:*
+
